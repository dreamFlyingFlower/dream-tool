package dream.flying.flower.nio;
...
/**
 * Buffer工具类,{@link java.nio.Buffer} FIXME
 * 
 * @author 飞花梦影
...
public class BufferHelper {
	...
	public static ByteBuffer create(CharSequence data) {
		return create(data.toString().getBytes(ConstCharset.DEFAULT_CHARSET));
	}
	...
	public static ByteBuffer create(CharSequence data, Charset charset) {
		return create(data.toString().getBytes(charset));
	}
	...
	public static String read(ByteBuffer buffer) {
		return read(buffer, ConstCharset.DEFAULT_CHARSET);
	}
	 ...
	 * @return 字符串
	 */
	public static String read(ByteBuffer buffer, Charset charset) {
		return StrHelper.toString(readBytes(buffer), charset);
	}
