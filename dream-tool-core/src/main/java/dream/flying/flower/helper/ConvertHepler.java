...
public class ConvertHepler {
	 ...
	 * 转换为boolean
	 * 
	 * 支持的值为:true、false、yes、ok、no,1,0 如果给定的值为空,或者转换失败,返回默认值
	 * 
	 * @param value 被转换的值
	 * @param defaultValue 转换错误时的默认值
	 * @return 结果
	...
	public static Character toChar(Object value, Character defaultValue) {
		...
		final String valueStr = toStr(value);
		return StrHelper.isBlank(valueStr) ? defaultValue : valueStr.charAt(0);
	}
	...
	public static String toStr(byte[] bytes, String charsetName) {
		return toStr(bytes, CharsetHelper.defaultCharset(charsetName));
	}
	 ...
	 * @return 字符串
	 */
	public static String toStr(ByteBuffer data, Charset charset) {
		if (null == charset) {
	 ...
	 * @return 字符串
	 */
	public static String toStr(ByteBuffer data, String charset) {
		if (data == null) {
			return null;
		}

		return toStr(data, Charset.forName(charset));
	}
