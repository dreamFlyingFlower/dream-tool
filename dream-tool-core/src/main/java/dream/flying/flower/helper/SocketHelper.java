...
/**
 * Socket工具类,{@link org.springframework.util.SocketUtils} FIXME
 * 
 * @author 飞花梦影
 * @date 2021-03-06 19:37:12
...
public class SocketHelper {
	...
	private enum SocketType {
		...
		int findAvailablePort(int minPort, int maxPort) {
			AssertHelper.isTrue(minPort > 0, "'minPort' must be greater than 0");
			AssertHelper.isTrue(maxPort >= minPort, "'maxPort' must be greater than or equal to 'minPort'");
			AssertHelper.isTrue(maxPort <= PORT_RANGE_MAX, "'maxPort' must be less than or equal to " + PORT_RANGE_MAX);
		...
		/**
		 * Find the requested number of available ports for this {@code SocketType}, each randomly selected
		...
		SortedSet<Integer> findAvailablePorts(int numRequested, int minPort, int maxPort) {
			AssertHelper.isTrue(minPort > 0, "'minPort' must be greater than 0");
			AssertHelper.isTrue(maxPort > minPort, "'maxPort' must be greater than 'minPort'");
			AssertHelper.isTrue(maxPort <= PORT_RANGE_MAX, "'maxPort' must be less than or equal to " + PORT_RANGE_MAX);
			AssertHelper.isTrue(numRequested > 0, "'numRequested' must be greater than 0");
			AssertHelper.isTrue((maxPort - minPort) >= numRequested,
