...
import com.dream.annotation.Nullable;
...
public class IterableHelper {
	...
	public static <T> boolean addAll(final Collection<T> collection, final Iterable<? extends T> iterable) {
		AssertHelper.notNull(collection);
	...
	public static boolean contains(Iterable<?> iterable, @Nullable Object element) {
		return contains(AssertHelper.notNull(iterable).iterator(), element);
	}
	...
	public static <T> void filter(Iterable<T> iterable, Consumer<T> consumer) {
		filter(AssertHelper.notNull(iterable).iterator(), consumer);
	}
	...
	public static <T> void filter(Iterable<T> iterable, Predicate<T> predicate) {
		filter(AssertHelper.notNull(iterable).iterator(), predicate);
	}
	...
	public static <T> T find(final Iterable<T> iterable, final Predicate<? super T> predicate) {
		return find(AssertHelper.notNull(iterable).iterator(), predicate);
	}
	...
	public static <T> T find(final Iterator<T> iterator, final Predicate<? super T> predicate) {
		AssertHelper.notNull(predicate);
		if (iterator != null) {
			while (iterator.hasNext()) {
				final T element = iterator.next();
	...
	public static <T> List<Object> getFieldList(Iterator<T> iterator, String fieldName)
		...
		if (null != iterator) {
			...
			while (iterator.hasNext()) {
				value = iterator.next();
				result.add(ReflectHelper.readField(value, fieldName));
			}
		}
		return result;
	}
	...
	public static <T> T getFirst(Iterable<T> iterable) {
		return getFirst(AssertHelper.notNull(iterable).iterator());
	}
	...
	public static <T, K, V> Map<K, List<V>> toListMap(Iterable<T> iterable, Function<T, K> keyFunction,
		...
		return toListMap(iterable, MapHelper.newHashMap(), keyFunction, valueFunction);
	}
	...
	public static <K, V> Map<K, V> toMap(Iterator<K> keys, Iterator<V> values, boolean order) {
		final Map<K, V> result = order ? MapHelper.newLinkedHashMap() : MapHelper.newHashMap();
		if (isNotEmpty(keys)) {
			while (keys.hasNext()) {
