...
public class HtmlTool {
	...
	public static String unescape(String htmlStr) {
		if (StrTool.isBlank(htmlStr)) {
			return htmlStr;
		}
		return EscapeTool.unescape(htmlStr);
	}
	...
	public static String removeHtmlTag(String content, boolean withTagContent, String... tagNames) {
		...
		for (String tagName : tagNames) {
			...
			content = RegularTool.removePattern(content, regex); // 非自闭标签小写
		}
		return content;
	}
	...
	public static String removeAllHtmlAttr(String content, String... tagNames) {
		String regex;
		for (String tagName : tagNames) {
			regex = StrTool.format("(?i)<{}[^>]*?>", tagName);
			content = content.replaceAll(regex, StrTool.format("<{}>", tagName));
		}
		return content;
	}
