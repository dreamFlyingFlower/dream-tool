...
import com.wy.primitive.CharHelper;
...
public class CharSequenceHelper {
	...
	public static boolean endsWithAny(final CharSequence cs, final CharSequence... searchStrs) {
		if (isEmpty(cs) || ArrayHelper.isEmpty(searchStrs)) {
			return false;
		}
	...
	public static boolean equalsAny(final CharSequence cs, final CharSequence... searchStrs) {
		if (ArrayHelper.isNotEmpty(searchStrs)) {
			for (final CharSequence searchStr : searchStrs) {
	...
	public static boolean isAsciiPrintable(final CharSequence cs) {
		...
		final int sz = cs.length();
		for (int i = 0; i < sz; i++) {
			if (!CharHelper.isAsciiPrintable(cs.charAt(i))) {
				return false;
			}
		}
		return true;
