...
public class AnnotationTool {
	...
	public static <A extends Annotation> A getAnnotation(final Method method, final Class<A> annotationCls,
		...
		AssertTool.notNull(method, "The method must not be null");
		AssertTool.notNull(annotationCls, "The annotation class must not be null");
		if (!ignoreAccess && !ReflectTool.isAccessible(method)) {
			return null;
		}
		A annotation = method.getAnnotation(annotationCls);
		if (annotation == null && searchSupers) {
			final Class<?> mcls = method.getDeclaringClass();
			final List<Class<?>> classes = ClassTool.getSuperclassesAndInterfaces(mcls);
			for (final Class<?> acls : classes) {
				final Method equivalentMethod = ReflectTool.getMethod(acls, method.getName(),
						method.getParameterTypes());
				if (equivalentMethod != null) {
					ReflectTool.fixAccessible(equivalentMethod, ignoreAccess);
