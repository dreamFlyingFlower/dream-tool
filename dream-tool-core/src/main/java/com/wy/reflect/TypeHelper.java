...
import com.wy.lang.ObjectHelper;
import com.wy.util.ArrayHelper;

/**
 * {@link Type}工具类,主要是类型检查以及泛型检查,未使用,来源于{@link org.apache.commons.lang3.reflect.TypeUtils}
 * 
 * {@link Class},{@link GenericArrayType},{@link TypeVariable},{@link ParameterizedType},{@link WildcardType}
 * 
 * @author 飞花梦影 FIXME
 * @date 2021-03-04 16:28:11
...
public class TypeHelper {
	...
	/**
	 * {@link GenericArrayType}实现类
	 *
	 * @author 飞花梦影
	 * @date 2021-03-05 22:53:39
	 * @git {@link https://github.com/dreamFlyingFlower}
	 */
	private static final class GenericArrayTypeImpl implements GenericArrayType {
		...
		public String toString() {
			return TypeHelper.toString(this);
		}

		@Override
		public boolean equals(final Object obj) {
			return obj == this || obj instanceof GenericArrayType && TypeHelper.equals(this, (GenericArrayType) obj);
		}
	...
	/**
	 * {@link ParameterizedType}实现类
	 * 
	 * @author 飞花梦影
	 * @date 2021-03-05 22:54:26
	 * @git {@link https://github.com/dreamFlyingFlower}
	 */
	private static final class ParameterizedTypeImpl implements ParameterizedType {
		...
		@Override
		public boolean equals(final Object obj) {
			return obj == this || obj instanceof ParameterizedType && TypeHelper.equals(this, ((ParameterizedType) obj));
		}
	...
	/**
	 * {@link WildcardType}实现类
	 *
	 * @author 飞花梦影
	 * @date 2021-03-05 22:54:50
	 * @git {@link https://github.com/dreamFlyingFlower}
	 */
	private static final class WildcardTypeImpl implements WildcardType {
		...
		private WildcardTypeImpl(final Type[] upperBounds, final Type[] lowerBounds) {
			this.upperBounds = ObjectHelper.defaultIfNull(upperBounds, ConstArray.EMPTY_TYPE);
			this.lowerBounds = ObjectHelper.defaultIfNull(lowerBounds, ConstArray.EMPTY_TYPE);
		}
		...
		@Override
		public boolean equals(final Object obj) {
			return obj == this || obj instanceof WildcardType && TypeHelper.equals(this, (WildcardType) obj);
		}
	 ...
	 * {@link ClassHelper#isAssignable(Class, Class)}.
	 * </p>
	 *
	...
	private static boolean isAssignable(final Type type, final Class<?> toClass) {
		if (type == null) {
			// consistency with ClassTool.isAssignable() behavior
		...
		if (type instanceof Class<?>) {
			...
			return ClassHelper.isAssignable((Class<?>) type, toClass);
		}
	...
	private static Map<TypeVariable<?>, Type> getTypeArguments(Class<?> cls, final Class<?> toClass,
		...
		if (cls.isPrimitive()) {
			...
			cls = ClassHelper.primitiveToWrapper(cls);
		}
	...
	public static Map<TypeVariable<?>, Type> determineTypeArguments(final Class<?> cls,
		...
		AssertHelper.notNull(cls, "cls is null");
		AssertHelper.notNull(superType, "superType is null");
	...
	public static Type[] normalizeUpperBounds(final Type[] bounds) {
		AssertHelper.notNull(bounds, "null value specified for bounds array");
	...
	public static Type[] getImplicitBounds(final TypeVariable<?> typeVariable) {
		AssertHelper.notNull(typeVariable, "typeVariable is null");
	...
	public static Type[] getImplicitUpperBounds(final WildcardType wildcardType) {
		AssertHelper.notNull(wildcardType, "wildcardType is null");
	...
	public static boolean typesSatisfyVariables(final Map<TypeVariable<?>, Type> typeVarAssigns) {
		AssertHelper.notNull(typeVarAssigns, "typeVarAssigns is null");
	...
	private static Type[] unrollBounds(final Map<TypeVariable<?>, Type> typeArguments, final Type[] bounds) {
		...
		for (; i < result.length; i++) {
			...
			if (unrolled == null) {
				result = ArrayHelper.remove(result, i--);
			} else {
				result[i] = unrolled;
			}
		}
	...
	public static final ParameterizedType parameterize(final Class<?> rawClass,
		...
		AssertHelper.notNull(rawClass, "raw class is null");
		AssertHelper.notNull(typeArgMappings, "typeArgMappings is null");
	...
	public static final ParameterizedType parameterizeWithOwner(final Type owner, final Class<?> rawClass,
		...
		if (rawClass.getEnclosingClass() == null) {
			AssertHelper.isTrue(owner == null, "no owner allowed for top-level %s", rawClass);
			useOwner = null;
		} else if (owner == null) {
			useOwner = rawClass.getEnclosingClass();
		} else {
			AssertHelper.isTrue(isAssignable(owner, rawClass.getEnclosingClass()),
					"%s is invalid owner type for parameterized %s", owner, rawClass);
			useOwner = owner;
		}
		AssertHelper.noNullElements(typeArguments, "null type argument at index %s");
		AssertHelper.isTrue(rawClass.getTypeParameters().length == typeArguments.length,
	...
	private static Type[] extractTypeArgumentsFrom(final Map<TypeVariable<?>, Type> mappings,
		...
		for (final TypeVariable<?> var : variables) {
			AssertHelper.isTrue(mappings.containsKey(var), "missing argument mapping for %s", toString(var));
	...
	public static GenericArrayType genericArrayType(final Type componentType) {
		AssertHelper.notNull(componentType, "componentType is null");
	...
	public static String toLongString(final TypeVariable<?> var) {
		AssertHelper.notNull(var, "var is null");
	 ...
	 * @return Typed&lt;T&gt;
	 */
	public static Supplier<Type> wrap(final Type type) {
		return () -> type;
	}
	 ...
	 * @return Typed&lt;T&gt;
	 */
	public static Supplier<Type> wrap(final Class<?> type) {
	...
	private static void appendRecursiveTypes(final StringBuilder builder, final int[] recursiveTypeIndexes,
		...
		// FIXME 减少循环,先排序或小循环在外
		for (int i = 0; i < argumentTypes.length; i++) {
			for (int j = 0; j < recursiveTypeIndexes.length; j++) {
				if (i == recursiveTypeIndexes[j]) {
					flag = false;
					count++;
					break;
				}
			...
			if (flag) {
				argumentsFiltered[i - count] = argumentTypes[i];
			} else {
				flag = true;
			}
		}
		...
		for (int i = 0; i < filteredArgumentTypes.length; i++) {
			if (filteredArgumentTypes[i] instanceof TypeVariable<?>) {
				if (containsVariableTypeSameParametrizedTypeBound(((TypeVariable<?>) filteredArgumentTypes[i]),
						parameterizedType)) {
					indexesToRemove = ArrayHelper.append(indexesToRemove, i);
				}
			}
		}
	...
	private static boolean containsVariableTypeSameParametrizedTypeBound(final TypeVariable<?> typeVariable,
		...
		return ArrayHelper.contains(typeVariable.getBounds(), parameterizedType);
	}
	...
	private static <T> StringBuilder appendAllTo(final StringBuilder builder, final String sep,
		...
		AssertHelper.noNullElements(types);
		if (types.length > 0) {
