...
public class ClassTool {
	...
	public static ClassLoader getClassLoader() {
		...
		if (cl == null) {
			// 使用当前类的类加载器
			cl = ClassTool.class.getClassLoader();
			if (cl == null) {
	...
	public static List<Class<?>> getSuperclassesAndInterfaces(final Class<?> clazz) {
		...
		final List<Class<?>> allSuperclasses = ClassTool.getSuperclasses(clazz);
		int superClassIndex = 0;
		final List<Class<?>> allInterfaces = ClassTool.getInterfaces(clazz);
	...
	public static boolean isAssignable(Class<?> subType, final Class<?> superType, boolean autoBox) {
		AssertTool.notNull(subType, "subType class must not be null");
		AssertTool.notNull(superType, "superType class must not be null");
