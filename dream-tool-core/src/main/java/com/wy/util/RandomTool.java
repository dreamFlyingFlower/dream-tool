...
public class RandomTool {
	...
	public static byte[] nextBytes(final int count) {
		AssertTool.isTrue(count >= 0, "Count cannot be negative.");
	...
	public static double nextDouble(final double start, final double end) {
		AssertTool.isTrue(end >= start, "Start value must be smaller or equal to end value.");
		AssertTool.isTrue(start >= 0, "Both range values must be non-negative.");
		if (start == end) {
			return start;
		}
